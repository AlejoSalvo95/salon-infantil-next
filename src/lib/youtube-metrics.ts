...
export type YouTubeChannelMetrics = { channel: string; shorts: YouTubeVideoMetric[]; videos: YouTubeVideoMetric[]; fetchedAt: string; classification: "duration" };
type ChannelIdentifier = { kind: "forHandle" | "id" | "forUsername"; value: string; canonical: string };
type ApiError = { error?: { message?: string } };
type ChannelResponse = ApiError & { items?: Array<{ contentDetails?: { relatedPlaylists?: { uploads?: string } } }> };
...
async function resolveUploadsPlaylist(identifier: ChannelIdentifier) { const response = await youtubeApi<ChannelResponse>("channels", { part: "contentDetails", [identifier.kind]: identifier.value }); const playlist = response.items?.[0]?.contentDetails?.relatedPlaylists?.uploads; if (!playlist) throw new Error("No se encontró el canal o su lista de publicaciones públicas."); return playlist; }
...
export async function getYouTubeChannelMetrics(input: string): Promise<YouTubeChannelMetrics> { const identifier = channelIdentifier(input); const items = await loadVideos(await loadVideoIds(await resolveUploadsPlaylist(identifier))); const shorts = items.filter((item) => item.duration !== null && item.duration <= 180); const videos = items.filter((item) => item.duration === null || item.duration > 180); return { channel: identifier.canonical, shorts, videos, fetchedAt: new Date().toISOString(), classification: "duration" }; }
