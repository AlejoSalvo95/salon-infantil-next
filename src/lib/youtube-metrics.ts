...
export type YouTubeChannelMetrics = { channel: string; country: string | null; subscriberCount: number | null; hiddenSubscriberCount: boolean; shorts: YouTubeVideoMetric[]; videos: YouTubeVideoMetric[]; fetchedAt: string; classification: "duration" };
...
async function resolveChannel(identifier: ChannelIdentifier) { const response = await youtubeApi<ChannelResponse>("channels", { part: "snippet,contentDetails,statistics", [identifier.kind]: identifier.value }); const channel = response.items?.[0], playlist = channel?.contentDetails?.relatedPlaylists?.uploads; if (!playlist) throw new Error("No se encontró el canal o su lista de publicaciones públicas."); return { playlist, country: channel?.snippet?.country ?? null, subscriberCount: count(channel?.statistics?.subscriberCount), hiddenSubscriberCount: channel?.statistics?.hiddenSubscriberCount ?? false }; }
