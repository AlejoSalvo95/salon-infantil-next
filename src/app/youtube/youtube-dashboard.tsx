...
function MetricTable({ title, accent, items }: { title: string; accent: "cyan" | "yellow"; items: YouTubeVideoMetric[] }) {
  ...
  return <section className={`youtube-results ${accent}`}><div className="youtube-results-heading"><div><p className="youtube-kicker">{items.length} publicaciones</p><h2>{title}</h2></div><span>{number.format(total(items, "viewCount"))} vistas</span></div>{items.length ? <>
    ...
    <PerformanceChart items={items} medianViews={medianViews}/>
    <div className="youtube-table-wrap"><table><thead><tr><th>Fecha</th><th>Título</th><th>Vistas · rendimiento</th><th>Engagement</th><th>Likes</th><th>Comentarios</th><th>Duración</th><th></th></tr></thead><tbody>{items.map((item, index) => { const multiplier = medianViews > 0 && item.viewCount !== null ? item.viewCount / medianViews : 0; const rating = performanceLabel(multiplier); const rate = engagement(item); return <tr key={`${item.url}-${index}`}><td>{formatDate(item.uploadDate)}</td><td><strong>{item.title}</strong></td><td><span className={`performance ${rating.className}`}>{rating.icon} {item.viewCount === null ? "—" : number.format(item.viewCount)} <small>{multiplier.toFixed(1)}x · {rating.label}</small></span></td><td>{rate === null ? "—" : `${rate.toFixed(2)}%`}</td><td>{item.likeCount === null ? "—" : number.format(item.likeCount)}</td><td>{item.commentCount === null ? "—" : number.format(item.commentCount)}</td><td>{formatDuration(item.duration)}</td><td><a href={item.url} target="_blank" rel="noreferrer" aria-label={`Abrir ${item.title} en YouTube`}>↗</a></td></tr>; })}</tbody></table></div>
...
function PerformanceChart({ items, medianViews }: { items: YouTubeVideoMetric[]; medianViews: number }) {
  const data = [...items].reverse().map((item) => ({ item, relative: medianViews > 0 && item.viewCount !== null ? item.viewCount / medianViews : 0, rate: engagement(item) ?? 0 }));
  const width = 1000, height = 350, left = 58, right = 58, top = 28, bottom = 48;
  const maxRelative = Math.max(1, ...data.map((point) => point.relative));
  ...
  const yRelative = (value: number) => top + (1 - value / maxRelative) * (height - top - bottom);
  const yRate = (value: number) => top + (1 - value / maxRate) * (height - top - bottom);
  const line = (key: "relative" | "rate", scale: (value: number) => number) => data.map((point, index) => `${index ? "L" : "M"}${x(index).toFixed(1)},${scale(point[key]).toFixed(1)}`).join(" ");
  const first = data[0]?.item.uploadDate ?? null, last = data.at(-1)?.item.uploadDate ?? null;
  return <div className="performance-chart"><div className="performance-chart-head"><div><p className="youtube-kicker">Evolución por publicación</p><h3>Rendimiento y engagement</h3></div><div className="performance-legend"><span><i className="relative-line"/>Rendimiento vs. mediana</span><span><i className="engagement-line"/>Engagement</span></div></div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Curvas de rendimiento relativo y engagement por publicación"><line className="performance-grid" x1={left} x2={width-right} y1={height-bottom} y2={height-bottom}/>{[0,.25,.5,.75,1].map((part) => <g key={part}><line className="performance-grid" x1={left} x2={width-right} y1={top+part*(height-top-bottom)} y2={top+part*(height-top-bottom)}/><text className="performance-axis left" x={left-10} y={top+(1-part)*(height-top-bottom)+4}>{(maxRelative*part).toFixed(1)}x</text><text className="performance-axis right" x={width-right+10} y={top+(1-part)*(height-top-bottom)+4}>{(maxRate*part).toFixed(1)}%</text></g>)}<path className="relative-path" d={line("relative", yRelative)}/><path className="engagement-path" d={line("rate", yRate)}/>{data.map((point, index) => <circle key={point.item.url} className="relative-dot" cx={x(index)} cy={yRelative(point.relative)} r="3"><title>{`${point.item.title}: ${point.relative.toFixed(1)}x`}</title></circle>)}{data.map((point, index) => <circle key={`e-${point.item.url}`} className="engagement-dot" cx={x(index)} cy={yRate(point.rate)} r="3"><title>{`${point.item.title}: ${point.rate.toFixed(2)}%`}</title></circle>)}<text className="performance-axis date start" x={left} y={height-13}>{formatDate(first)}</text><text className="performance-axis date end" x={width-right} y={height-13}>{formatDate(last)}</text></svg></div>;
}
