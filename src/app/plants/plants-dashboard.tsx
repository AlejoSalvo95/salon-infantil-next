...
function GrowthChart({ measurements, water, nutrients }: { measurements: PlantMeasurement[]; water: PlantEvent[]; nutrients: PlantEvent[] }) {
  ...
  return <section className="growth"><div className="chart-heading"><div><p className="plants-kicker">Evolución temporal</p><h2>Altura total</h2></div><div className="chart-legend"><span><i className="legend-line"/>Crecimiento sin riego</span><span><i className="legend-water"/>Tramo con riego</span><span><i className="legend-nutrient"/>Nutrientes</span></div></div><div className="chart-value" aria-live="polite"><strong>{compact.format(active.totalHeight)} <small>altura</small></strong>{activeWater !== undefined && <strong className="water-value">{compact.format(activeWater)} <small>agua agregada</small></strong>}<span>{dateFormat.format(new Date(`${active.date}T12:00`))}</span></div><div className="chart-wrap" ref={wrap}><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Gráfica de altura total con tramos azules en ${dailyWater.length} fechas de riego`}><defs><linearGradient id="growth-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="var(--mint)" stopOpacity=".8"/><stop offset="1" stopColor="var(--mint)" stopOpacity=".05"/></linearGradient></defs>{yTicks.map((tick) => <g key={tick}><line className="chart-grid" x1={left} x2={width-right} y1={y(tick)} y2={y(tick)}/><text className="chart-axis y" x={left-10} y={y(tick)+4}>{compact.format(tick)}</text></g>)}<text className="axis-title height-title" x={left} y={14}>Altura total</text><path className="chart-area" d={area}/><path className="chart-line" d={line}/>{measurements.slice(1).map((item, index) => waterByDate.has(item.date) ? <line key={item.date} className="water-height-segment" x1={x(timestamp(measurements[index].date))} y1={y(measurements[index].totalHeight)} x2={x(timestamp(item.date))} y2={y(item.totalHeight)}/> : null)}{dailyWater.map((item) => { const measurement = measurements.find((entry) => entry.date === item.date); return measurement ? <circle key={item.date} className="water-height-point" cx={x(timestamp(item.date))} cy={y(measurement.totalHeight)} r="4"/> : null})}{nutrientDates.map((date) => <path key={`n-${date}`} className="event-dot nutrient" d={`M${x(timestamp(date))},${height-bottom+27} l5,5 -5,5 -5,-5 Z`}/>)}{xTickIndexes.map((index) => <text key={index} className="chart-axis x" x={x(times[index])} y={height-12}>{new Intl.DateTimeFormat("es-UY", { month: "short", year: "2-digit" }).format(new Date(`${measurements[index].date}T12:00`))}</text>)}<line className="hover-guide" x1={x(timestamp(active.date))} x2={x(timestamp(active.date))} y1={top} y2={height-bottom}/><circle className={activeWater === undefined ? "hover-dot" : "water-height-hover"} cx={x(timestamp(active.date))} cy={y(active.totalHeight)} r="6"/><rect className="chart-hit" x={left} y={top} width={width-left-right} height={height-top-bottom} onPointerMove={move} onPointerLeave={() => setHovered(null)}/></svg></div><p className="chart-note">Toda la curva representa altura. Los tramos azules terminan en una medición tomada un día con agua agregada; al pasar el cursor se muestra su cantidad.</p></section>;
}
...
function NaturalGrowthChart({ measurements, water }: { measurements: PlantMeasurement[]; water: PlantEvent[] }) {
  ...
  return <section className="growth natural-growth"><div className="chart-heading"><div><p className="plants-kicker">Cambio entre mediciones</p><h2>Crecimiento y decaimiento</h2></div><div className="chart-legend"><span><i className="legend-positive"/>Crecimiento</span><span><i className="legend-negative"/>Decaimiento</span></div></div><div className="chart-value" aria-live="polite"><strong className={active.value >= 0 ? "positive-value" : "negative-value"}>{active.value >= 0 ? "+" : ""}{compact.format(active.value)} <small>variación</small></strong><span>{dateFormat.format(new Date(`${active.date}T12:00`))}</span></div><div className="chart-wrap" ref={wrap}><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Variación de altura en ${changes.length} fechas sin eventos de agua`}>{yTicks.map((tick) => <g key={tick}><line className={tick === 0 ? "zero-line" : "chart-grid"} x1={left} x2={width-right} y1={y(tick)} y2={y(tick)}/><text className="chart-axis y" x={left-10} y={y(tick)+4}>{tick > 0 ? "+" : ""}{compact.format(tick)}</text></g>)}{changes.map((item) => { const itemY = y(item.value); return <rect key={item.date} className={item.value >= 0 ? "change-bar positive" : "change-bar negative"} x={x(timestamp(item.date))-barWidth/2} y={Math.min(itemY,zeroY)} width={barWidth} height={Math.max(Math.abs(zeroY-itemY),1)}/>})}{xTickIndexes.map((index) => <text key={index} className="chart-axis x" x={x(times[index])} y={height-12}>{new Intl.DateTimeFormat("es-UY", { month: "short", year: "2-digit" }).format(new Date(`${changes[index].date}T12:00`))}</text>)}<line className="hover-guide" x1={x(timestamp(active.date))} x2={x(timestamp(active.date))} y1={top} y2={height-bottom}/><circle className={active.value >= 0 ? "change-hover positive" : "change-hover negative"} cx={x(timestamp(active.date))} cy={y(active.value)} r="6"/><rect className="chart-hit" x={left} y={top} width={width-left-right} height={height-top-bottom} onPointerMove={move} onPointerLeave={() => setHovered(null)}/></svg></div><p className="chart-note">Se excluyen todas las fechas que tienen un evento de agua registrado. Cada barra muestra la diferencia respecto de la medición inmediatamente anterior.</p></section>;
}
...
function MonthlyGrowthCharts({ measurements, water }: { measurements: PlantMeasurement[]; water: PlantEvent[] }) {
  ...
  return <section className="monthly-section"><MonthlyBarChart title="Crecimiento mensual" kicker="Acumulado sin riego" data={absolute} valueLabel="cambio" formatter={(value) => `${value >= 0 ? "+" : ""}${compact.format(value)}`} note="Suma las variaciones de altura de cada mes, excluyendo las fechas con agua agregada."/><MonthlyBarChart title="Crecimiento porcentual" kicker="Comparación mes a mes" data={percentage} valueLabel="variación mensual" formatter={(value) => `${value >= 0 ? "+" : ""}${value.toLocaleString("es-UY", { maximumFractionDigits: 1 })}%`} note="Expresa el cambio mensual sin riego como porcentaje de la altura registrada al comenzar ese mes."/></section>;
}
