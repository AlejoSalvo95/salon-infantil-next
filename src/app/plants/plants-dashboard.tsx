...
export function PlantsDashboard({ measurements, waterEvents, nutrientEvents }: Props) {
  ...
  selected.slice(1).forEach((item, index) => {
    ...
    monthlyGrowth.set(month, (monthlyGrowth.get(month) ?? 0) + item.totalHeight - selected[index].totalHeight);
  });
  const averageMonthlyGrowth = monthlyGrowth.size
...
function CumulativeChangeChart({ measurements, water }: { measurements: PlantMeasurement[]; water: PlantEvent[] }) {
  const wrap = useRef<HTMLDivElement>(null);
  const [width, setWidth] = useState(900);
  const [hovered, setHovered] = useState<number | null>(null);
  useEffect(() => { const node = wrap.current; if (!node) return; const observer = new ResizeObserver(([entry]) => setWidth(Math.max(320, entry.contentRect.width))); observer.observe(node); return () => observer.disconnect(); }, []);
  ...
  const points = measurements.map((item, index) => {
    if (index > 0 && !wateredDates.has(item.date)) cumulative += item.totalHeight - measurements[index - 1].totalHeight;
    return { date: item.date, value: cumulative, watered: wateredDates.has(item.date) };
  });
  if (points.length < 2) return null;
  const height = width < 600 ? 340 : 410, left = width < 600 ? 48 : 70, right = 20, top = 24, bottom = 58;
  const times = points.map((item) => timestamp(item.date)), minX = Math.min(...times), maxX = Math.max(...times);
  const rawMin = Math.min(0, ...points.map((item) => item.value)), rawMax = Math.max(0, ...points.map((item) => item.value));
  const padding = Math.max((rawMax - rawMin) * .1, 1), minY = rawMin - padding, maxY = rawMax + padding;
  const x = (value: number) => left + ((value - minX) / Math.max(maxX - minX, 1)) * (width - left - right);
  const y = (value: number) => top + (1 - (value - minY) / Math.max(maxY - minY, 1)) * (height - top - bottom);
  const line = points.map((item, index) => `${index ? "L" : "M"}${x(timestamp(item.date)).toFixed(1)},${y(item.value).toFixed(1)}`).join(" ");
  const area = `${line} L${x(maxX)},${y(0)} L${x(minX)},${y(0)} Z`;
  const yTicks = Array.from({ length: 5 }, (_, index) => minY + ((maxY - minY) * index) / 4);
  const xTickIndexes = Array.from(new Set([0, Math.round((points.length - 1) / 3), Math.round((points.length - 1) * 2 / 3), points.length - 1]));
  const active = hovered === null ? points.at(-1)! : points[hovered];
  function move(event: React.PointerEvent<SVGRectElement>) { const bounds = event.currentTarget.getBoundingClientRect(); const cursor = ((event.clientX - bounds.left) / bounds.width) * width; let nearest = 0, distance = Infinity; points.forEach((item, index) => { const next = Math.abs(x(timestamp(item.date)) - cursor); if (next < distance) { distance = next; nearest = index; } }); setHovered(nearest); }
  return <section className="growth cumulative-growth"><div className="chart-heading"><div><p className="plants-kicker">On days without watering</p><h2>Cumulative change</h2></div><div className="chart-legend"><span><i className="legend-cumulative"/>Cumulative change</span><span><i className="legend-water-point"/>Watering day</span></div></div><div className="chart-value" aria-live="polite"><strong className={active.value >= 0 ? "positive-value" : "negative-value"}>{active.value >= 0 ? "+" : ""}{compact.format(active.value)} <small>cumulative change</small></strong><span>{dateFormat.format(new Date(`${active.date}T12:00`))}{active.watered ? " · watering day (unchanged)" : ""}</span></div><div className="chart-wrap" ref={wrap}><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Daily cumulative height change across ${points.length} observations, excluding watering days`}><defs><linearGradient id="cumulative-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#528166" stopOpacity=".26"/><stop offset="1" stopColor="#528166" stopOpacity=".02"/></linearGradient></defs>{yTicks.map((tick) => <g key={tick}><line className={Math.abs(tick) < .0001 ? "zero-line" : "chart-grid"} x1={left} x2={width-right} y1={y(tick)} y2={y(tick)}/><text className="chart-axis y" x={left-10} y={y(tick)+4}>{tick > 0 ? "+" : ""}{compact.format(tick)}</text></g>)}<line className="zero-line" x1={left} x2={width-right} y1={y(0)} y2={y(0)}/><path className="cumulative-area" d={area}/><path className="cumulative-line" d={line}/>{points.filter((item) => item.watered).map((item) => <circle key={item.date} className="cumulative-water-point" cx={x(timestamp(item.date))} cy={y(item.value)} r="3.5"/>)}{xTickIndexes.map((index) => <text key={index} className="chart-axis x" x={x(times[index])} y={height-14}>{new Intl.DateTimeFormat("en-US", { month: "short", year: "2-digit" }).format(new Date(`${points[index].date}T12:00`))}</text>)}<line className="hover-guide" x1={x(timestamp(active.date))} x2={x(timestamp(active.date))} y1={top} y2={height-bottom}/><circle className="cumulative-hover" cx={x(timestamp(active.date))} cy={y(active.value)} r="6"/><rect className="chart-hit" x={left} y={top} width={width-left-right} height={height-top-bottom} onPointerMove={move} onPointerLeave={() => setHovered(null)}/></svg></div><p className="chart-note">Each point shows the running total on that date. Changes measured on watering days are excluded, so the line stays flat for those observations.</p></section>;
}
